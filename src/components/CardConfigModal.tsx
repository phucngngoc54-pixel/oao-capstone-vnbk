...
import { X, Save, Smartphone, CheckCircle2, ChevronRight, Building2, ArrowLeft, Activity, ExternalLink, Copy, Upload, EyeOff } from 'lucide-react';
...
export default function CardConfigModal({ isOpen, onClose, configId, data, onSave, onRedirectToPartners }: CardConfigModalProps) {
    ...
    const addSubContent = () => {
        ...
        setProductDetail({ ...productDetail, Sub_Contents: [...(productDetail.Sub_Contents || []), newItem] });
    };
    ...
    const addMainCta = () => {
        ...
        setProductDetail({ ...productDetail, Main_CTAs: [...(productDetail.Main_CTAs || []), newItem] });
    };
                        ...
                        <div className="flex-1 overflow-y-auto p-6 bg-slate-100 space-y-6">
                            ...
                            <div className="bg-[#f8fafc] border border-slate-200 rounded-xl p-6 shadow-sm">
                                ...
                                <div className={`space-y-8 transition-opacity ${cardUI.Has_Detail_Block ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
                                    ...
                                    <div className="p-4 bg-white border border-slate-200 rounded-lg">
                                        ...
                                        <label className="block text-[11px] font-bold text-slate-500 uppercase mb-1.5">Nội dung (Quyền lợi & Điều kiện)</label>
                                        <textarea
                                    ...
                                    <div className="p-4 bg-white border border-slate-200 rounded-lg">
                                        <div className="flex justify-between items-center mb-4">
                                            ...
                                            <button onClick={addSubContent} className="text-xs font-bold text-blue-600 hover:text-blue-800 flex items-center gap-1">+ Add Link</button>
                                        </div>
                                        <div className="space-y-3">
                                            {(productDetail.Sub_Contents || []).map((item, idx) => (
                                                <div key={item.id} className="p-3 border border-slate-100 bg-slate-50 rounded-lg relative">
                                                    <button onClick={() => removeSubContent(idx)} className="absolute top-2 right-2 text-slate-400 hover:text-red-500"><X size={14} /></button>
                                                    <div className="grid grid-cols-2 gap-3 pr-6">
                                                        <div>
                                                            <label className="block text-[10px] font-bold text-slate-500 mb-1">Label</label>
                                                            <input type="text" value={item.label} onChange={e => updateSubContent(idx, 'label', e.target.value)} className="w-full p-2 text-xs border rounded-md" placeholder="Xem điều khoản..." />
                                                        </div>
                                                        <div>
                                                            <label className="block text-[10px] font-bold text-slate-500 mb-1">ZPI Link</label>
                                                            <input type="text" value={item.zpi_link || ''} onChange={e => updateSubContent(idx, 'zpi_link', e.target.value)} className="w-full p-2 text-xs border rounded-md" placeholder="https://..." />
                                                        </div>
                                                    </div>
                                                </div>
                                            ))}
                                            {(!productDetail.Sub_Contents || productDetail.Sub_Contents.length === 0) && (
                                                <p className="text-xs text-slate-400 text-center py-2">No sub-contents currently. Click "+ Add Link" to create one.</p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-4 bg-white border border-slate-200 rounded-lg">
                                        <div className="flex justify-between items-center mb-4">
                                            <h4 className="text-xs font-bold text-slate-700 uppercase">Guidances</h4>
                                            <button onClick={addGuidance} className="text-xs font-bold text-blue-600 hover:text-blue-800 flex items-center gap-1">+ Add Guidance</button>
                                        </div>
                                        <div className="space-y-3">
                                            {(productDetail.Guidances || []).map((item, idx) => (
                                                <div key={item.id} className="p-3 border border-slate-100 bg-slate-50 rounded-lg relative flex gap-3">
                                                    <button onClick={() => removeGuidance(idx)} className="absolute top-2 right-2 text-slate-400 hover:text-red-500 z-10"><X size={14} /></button>
                                                    <div className="flex-1">
                                                        <label className="block text-[10px] font-bold text-slate-500 mb-1">Content (Step {idx + 1})</label>
                                                        <input type="text" value={item.content} onChange={e => updateGuidance(idx, 'content', e.target.value)} className="w-full p-2 text-xs border rounded-md mb-2" placeholder="Đăng ký tài khoản..." />
                                                        <label className="block text-[10px] font-bold text-slate-500 mb-1">Image URL</label>
                                                        <input type="text" value={item.image_url} onChange={e => updateGuidance(idx, 'image_url', e.target.value)} className="w-full p-2 text-xs border rounded-md pr-8" placeholder="https://..." />
                                                    </div>
                                                    {item.image_url && (
                                                        <div className="w-20 shrink-0 border rounded-md bg-white overflow-hidden flex items-center justify-center">
                                                            <img src={item.image_url} alt="preview" className="max-w-full max-h-full object-contain" />
                                                        </div>
                                                    )}
                                                </div>
                                            ))}
                                            {(!productDetail.Guidances || productDetail.Guidances.length === 0) && (
                                                <p className="text-xs text-slate-400 text-center py-2">No guidances. Click "+ Add Guidance" to start.</p>
                                            )}
                                        </div>
                                    </div>
                            ...
                            <div className="bg-[#f8fafc] border border-slate-200 rounded-xl p-6 shadow-sm">
                                <div className="flex justify-between items-center mb-6">
                                    <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Main CTA</h3>
                                    <button onClick={addMainCta} className="text-xs font-bold text-blue-600 hover:text-blue-800 flex items-center gap-1">+ Add CTA</button>
                                </div>
                                <div className="space-y-4">
                                    {(productDetail.Main_CTAs || []).map((cta, idx) => (
                                        <div key={cta.id} className="p-4 border border-slate-200 bg-white rounded-lg relative">
                                            <button onClick={() => removeMainCta(idx)} className="absolute top-3 right-3 text-slate-400 hover:text-red-500"><X size={16} /></button>

                                            <div className="grid grid-cols-2 gap-4 pr-6 mb-4">
                                                <div>
                                                    <label className="block text-[10px] font-bold text-slate-500 mb-1">Condition</label>
                                                    <select value={cta.condition} onChange={e => updateMainCta(idx, 'condition', e.target.value)} className="w-full p-2 text-xs border rounded-md bg-slate-50">
                                                        <option value="CONFIRM_CONDITION">CONFIRM_CONDITION</option>
                                                        <option value="NOT_ELIGIBLE">NOT_ELIGIBLE</option>
                                                        <option value="DEFAULT">DEFAULT</option>
                                                    </select>
                                                </div>
                                                <div>
                                                    <label className="block text-[10px] font-bold text-slate-500 mb-1">Action Type</label>
                                                    <select value={cta.action_type} onChange={e => updateMainCta(idx, 'action_type', e.target.value)} className="w-full p-2 text-xs border rounded-md bg-slate-50">
                                            ...
                                            <div className="grid grid-cols-2 gap-4 mb-4">
                                                <div>
                                                    ...
                                                    <input type="text" value={cta.name} onChange={e => { updateMainCta(idx, 'name', e.target.value); if (idx === 0) setCardUI({ ...cardUI, CTA_Label_Card: e.target.value }) }} className="w-full p-2 text-xs border rounded-md" placeholder="Mở ngay" />
                                                </div>
                                                <div>
                                                    <label className="block text-[10px] font-bold text-slate-500 mb-1">Description</label>
                                                    <input type="text" value={cta.description || ''} onChange={e => updateMainCta(idx, 'description', e.target.value)} className="w-full p-2 text-xs border rounded-md" placeholder="Input description here..." />
                                                </div>
                                                <div>
                                                    <label className="block text-[10px] font-bold text-slate-500 mb-1">URL (ZPI / Primary)</label>
                                                    <input type="text" value={cta.primary_url || ''} onChange={e => { updateMainCta(idx, 'primary_url', e.target.value); if (idx === 0) setProductDetail({ ...productDetail, ZPI_Link: e.target.value }) }} className="w-full p-2 text-xs border rounded-md" placeholder="https://..." />
                                                </div>
                                                <div>
                                                    <label className="block text-[10px] font-bold text-slate-500 mb-1">URL (ZPA / Secondary)</label>
                                                    <input type="text" value={cta.secondary_url || ''} onChange={e => updateMainCta(idx, 'secondary_url', e.target.value)} className="w-full p-2 text-xs border rounded-md" placeholder="https://..." />
                                                </div>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <div className="relative">
                                                    <input type="text" value={cta.extra_info_json || '{}'} onChange={e => updateMainCta(idx, 'extra_info_json', e.target.value)} className="w-full p-2 text-xs border rounded-md font-mono bg-slate-50" />
                                                    <span className="absolute right-3 top-2.5 text-[9px] text-emerald-500 font-bold">✓ Valid JSON</span>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                                    {(!productDetail.Main_CTAs || productDetail.Main_CTAs.length === 0) && (
                                        <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-center">
                                            <p className="text-xs text-yellow-700 mb-2">No CTAs defined. For legacy compatibility, basic CTA inputs are preserved below, but migrating to the dynamic list is recommended.</p>

                                            {/* Legacy fallback inputs if list is empty */}
                                            <div className="grid grid-cols-2 gap-x-8 gap-y-4 text-left mt-4 border-t border-yellow-200/50 pt-4">
                                                <div>
                                                    <label className="block text-[11px] font-bold text-slate-500 uppercase mb-1.5 ml-1">Button name <span className="text-red-500">*</span></label>
                                                    <input type="text" value={cardUI.CTA_Label_Card || ''} onChange={e => setCardUI({ ...cardUI, CTA_Label_Card: e.target.value })} className="w-full p-2.5 bg-white border border-slate-200 rounded-lg text-sm" />
                                                </div>
                                                <div>
                                                    <label className="block text-[11px] font-bold text-slate-500 uppercase mb-1.5 ml-1">ZPI Url <span className="text-red-500">*</span></label>
                                                    <input type="text" value={productDetail.ZPI_Link || ''} onChange={e => setProductDetail({ ...productDetail, ZPI_Link: e.target.value })} className="w-full p-2.5 bg-white border border-slate-200 rounded-lg text-sm" placeholder="https://mini.zalo.me/..." />
                                                </div>
                                            </div>
                                        </div>
                                    )}
